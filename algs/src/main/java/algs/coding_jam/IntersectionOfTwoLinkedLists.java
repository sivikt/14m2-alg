...
// 	two lists intersect). From the head of A, it reads as [0,9,1,2,4]. From the head of B, 
// it reads as [3,2,4]. There are 3 nodes before the intersected node in A; There are 1 node 
...
public class IntersectionOfTwoLinkedLists {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        for (int i = 0; i < Math.max(0, countA-countB); i++)
            headA = headA.next;
    
        for (int i = 0; i < Math.max(0, countB-countA); i++)
            headB = headB.next;
